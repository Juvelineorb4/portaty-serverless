...
def custom_graphql(query, variables=[]):
    client = boto3.client('appsync', region_name='us-east-1')  # Reemplaza 'us-east-1' con tu región
    api_id = 'ehid2xtqobf75gakml3qgdj45m'  # Reemplaza con el ID de tu API de AppSync

    endpoint_url = f'https://vkrtsagkivgqjgzbnhekkyusfm.appsync-api.{client.meta.region_name}.amazonaws.com/graphql'
    ...
    try:
        response = requests.post(endpoint_url, json=payload, headers=headers).json()
    ...
    try:
        for record in records:
            ...
            if "/incoming/" in decoded_key:
                ...
                print(f"Nombre de Bucket: {bucketName}, key: {decoded_key}, path: {pathImage}")
                ...
                if action == "create":
                    ...
                    if type == "extras":
                        ...
                        imageData = resize_image(BytesIO(original), (1024, 1024))
                        imageKey = f"{pathImage}/extras/{imageName}"
                        image = {"key": keyI, "url":f"https://{bucketName}.s3.amazonaws.com/{imageKey}" }
                        newArray = imagesObject
                        newArray.append(image)
                        s3.put_object(Body=imageData, Bucket=bucketName, Key=imageKey, ContentType="image/jpeg", Metadata={"businessid": businessid})
                        put_db_extras(businessid, [json.dumps(objeto) for objeto in newArray])
                    elif type == "profile":
                        # redimensionamos la imagen 
                        # Crear miniatura (360x360)
                        thumbnailData = resize_image(BytesIO(original), (360, 360))
                        thumbnailKey = f"{pathImage}/${imageName}_thumbnail.jpg"
                           # crear standar (1024x1024)
                        standardData = resize_image(BytesIO(original), (1024, 1024))
                        ...
                        s3.put_object(Body=standardData, Bucket=bucketName, Key=standardKey, ContentType="image/jpeg", Metadata={"businessid": businessid})

                        thumbnail = f"https://{bucketName}.s3.amazonaws.com/{thumbnailKey}"
                        image = {"key": keyI, "url":f"https://{bucketName}.s3.amazonaws.com/{standardKey}" }
