...
def handler(event, context):
    try:
        ...
        statusOwner = params.get("statusOwner", "NOT_ASSIGNED")  # El campo country es opcional

        # Construcción de la consulta base para negocios con 'statusOwner = NOT_ASSIGNED'
        query = {
            "size": 0,  # No necesitamos los documentos, solo agregaciones
            "query": {
                "bool": {
                    "must": [
                        {"match": {"statusOwner.keyword": statusOwner}}  # Filtro por statusOwner
                    ]
                }
            },
            "aggs": {
