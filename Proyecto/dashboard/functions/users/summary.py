...
# Cliente S3 para obtener el geojson del país
...
def handler(event, context):
    try:
        ...
        country = params.get("country", None)  # El campo country es opcion
        ...
        query = {
            ...
            "aggs": {
                "total_users": {
                    "value_count": {
                        "field": "id.keyword"  # Asegúrate de usar el campo 'keyword' para el conteo
                    }
                },
                "today_users": {
                    "filter": {
                        "range": {
                            "createdAt": {
                                "gte": "now/d"  # Usuarios creados desde hoy a medianoche UTC
                            }
                        }
                    }
                },
                "yesterday_users": {
                    "filter": {
                        "range": {
                            "createdAt": {
                                "gte": "now-1d/d",
                                "lt": "now/d"  # Usuarios creados ayer
                            }
                        }
                    }
                },
                ...
                "last_12_months_users": {
                    "filter": {
                        "range": {
                            "createdAt": {
                                "gte": "now-12M/d"  # Usuarios creados en los últimos 12 meses
                            }
                        }
                    }
                }
        ...
        total_users = response_data["aggregations"]["total_users"]["value"]
        today_users = response_data["aggregations"]["today_users"]["doc_count"]
        yesterday_users = response_data["aggregations"]["yesterday_users"]["doc_count"]
        last_7_days_users = response_data["aggregations"]["last_7_days_users"]["doc_count"]
        last_30_days_users = response_data["aggregations"]["last_30_days_users"]["doc_count"]
        last_12_months_users = response_data["aggregations"]["last_12_months_users"]["doc_count"]
        ...
        return {
            ...
            'body': json.dumps({
                ...
                },
                {
                    ...
                    "total": last_30_days_users
                },
                {
                    "title":"Ultimos 12 meses",
                    "total": last_12_months_users
                },
                ]
            })
        }
