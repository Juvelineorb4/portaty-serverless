...
def handler(event, context):
    try:
        ...
        print("PARAMS: ", params)
        period = params.get("range", "30D")  # Puede ser '7D', '30D', '12M'
        country = params.get("country", None)  # Campo opcional de paí
        ...
        if period == "7D":
            ...
            print(min_date)
            # min_date = (current_date - timedelta(days=365)).strftime('%Y-%m')
            max_date = current_date.strftime('%Y-%m')
            message = "Últimos 12 meses de usuarios obtenidos con éxito."
        ...
        buckets = response_data["aggregations"]["users_per_period"]["buckets"]
